resource "aws_instance" "mis_vms" {
  ...
  tags                        = { Name = "srv-${var.server_master}" }
...
data "template_file" "userdata_linux_ubuntu" {
  template = <<-EOT
                #!/bin/bash
                INICIO=$(date "+%F %H:%M:%S")
                echo "Hora de inicio del script: $INICIO" > /home/ubuntu/a_${var.server_master}.txt
                ...
                hostnamectl set-hostname ${var.server_master}
                ...
                echo "El rol de este servidor es: ${var.server_master}" > /home/ubuntu/b_${var.server_master}.txt
                FINAL=$(date "+%F %H:%M:%S")
                echo "Hora de finalizacion del script: $FINAL" >> /home/ubuntu/a_${var.server_master}.txt
